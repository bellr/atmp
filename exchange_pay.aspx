...
//echo $db->open_kod($demand_info[0]['order_arr'],$_POST['did']);

if($_POST['LMI_SECRET_KEY'] == "16201986") {
//вывод инфы по заявке
	$demand_info = $db->demand_check($_POST['did'],'WMB');
if (!empty($_POST['LMI_HASH'])) {
	$purse = $db->RezervPusre($demand_info[0]['company'],$WMB,$rezerv_WMB);
	...
	//echo $demand_info[0]['status'];
		//функция извлечения пин-кодов
		if($demand_info[0]['status'] == "n") {
			$db->open_kod($demand_info[0]['order_arr'],$_POST['did']);
...
$parser = new WMXIParser();
	$response = $wmxi->X2(intval($_POST['LMI_PAYMENT_NO']),$WMZ,$zalog_info[0]['purse_pay'],floatval($demand_info[0]['guarantee_price']),'0','','Возврат залога по заявке №'.$_POST['did'],'0');
